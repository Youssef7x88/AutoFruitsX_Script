...
teleSec:CreateDropdown("Island Selector", {"Starter Island", "Jungle", "Desert", "Skylands", "Second Sea", "Third Sea", "Floating Turtle", "Hydra Island", "Great Tree", "Castle on the Sea"}, function(selected)
    print("Teleporting to:", selected)
end)
teleSec:CreateButton("Teleport to Factory", function()
    LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(448.6, 199.1, -441.2)
end)
teleSec:CreateButton("Teleport to Safe Zone", function()
    LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(-3000, 200, -3000)
end)
teleSec:CreateButton("Teleport to Sea Beast Area", function()
    LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(13878, 200, -2000)
end)
...
raidSec:CreateDropdown("Raid Strategy", {"Kill All Bosses", "Drop in Sea"}, function(method)
    _G.raidMethod = method
end)
...
statsSec:CreateButton("Max All Stats", function()
...
miscSec:CreateToggle("FPS Boost", function(v)
    if v then
...
miscSec:CreateButton("Respawn Instantly", function()
    LocalPlayer:Kick("Respawning...")
end)
miscSec:CreateToggle("Anti AFK", function(v)
    if v then
        game:GetService("VirtualUser"):Button2Down(Vector2.new(0,0),workspace.CurrentCamera.CFrame)
    end
end)
miscSec:CreateButton("Reset Character", function()
    LocalPlayer.Character:BreakJoints()
end)

-- Settings
local setSec = settingsTab:CreateSection("General")
setSec:CreateButton("Save Config", function()
    local settings = {
        AutoLevel = _G.autoLevel,
        AutoQuest = _G.autoQuest,
    }
    writefile("AFX_Config.json", HttpService:JSONEncode(settings))
end)
setSec:CreateButton("Load Config", function()
    if isfile("AFX_Config.json") then
        local settings = HttpService:JSONDecode(readfile("AFX_Config.json"))
        for k, v in pairs(settings) do
            _G[k] = v
        end
    end
end)
setSec:CreateButton("Toggle UI", function()
    Window:Toggle()
end)
